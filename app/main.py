...
async def call_groq(system_prompt: str, user_message: str, temperature: float = 0.6, max_tokens: int = 400) -> str:
    if not GROQ_API_KEY:
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                delay *= 2
                continue
            raise HTTPException(status_code=502, detail=f"Failed to reach Groq: {e}")
    raise HTTPException(status_code=502, detail="Groq API is temporarily unavailable.")
...
async def improve(req: ImproveRequest, request: Request) -> ImproveResponse:
    ip = get_ip(request)
    key = ("improve", req.message, req.tone, req.platform, req.conversationContext, req.recipient)
    ...
    try:
        content = await call_groq(build_improve_prompt(req), req.message, temperature=0.6, max_tokens=400)
...
async def analyze(req: AnalyzeRequest, request: Request) -> AnalyzeResponse:
    ...
    try:
        content = await call_groq(system_prompt, req.message, temperature=0.3, max_tokens=100)
        parsed = json.loads(content)
...
async def reply(req: ReplyRequest, request: Request) -> ReplyResponse:
    ip = get_ip(request)
    key = ("reply", req.message, req.tone, req.platform, req.conversationContext, req.recipient)
